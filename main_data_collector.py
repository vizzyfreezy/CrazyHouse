...
def main():
    ...
    print(
        f"Fetching tournaments created by {USERNAME} starting from {datetime.fromtimestamp(START_TIMESTAMP / 1000, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S %Z')}..."
    )
    ...
    print(
        f"Successfully processed {processed_tournaments_count} tournaments meeting all criteria."
    )
...
if __name__ == "__main__":
    ...
    def load_from_cache(tournament_id):
        ...
        if os.path.exists(cache_file):
            try:
                ...
                is_finished_in_cache = (
                    cached_data.get("finished", False)
                    or cached_data.get("status") == "finished"
                    or cached_data.get("isFinished", False)
                )
                if is_finished_in_cache:
                    return cached_data
            except Exception:
                pass  # Simplified error handling for brevity
        return None
    ...
    def fetch_tournament_details(tournament_id):
        ...
        if nbPlayers == 0 or not standing.get("players"):
            if (
                tournament_data.get("finished", False)
                or tournament_data.get("status") == "finished"
                or tournament_data.get("isFinished", False)
            ):
        ...
        while current_api_page < total_pages:
            ...
            try:
                ...
                break
            next_page_players_list = next_page_data.get("standing", {}).get(
                "players", []
            )
        ...
        unique_players = [
            ...
            if p.get("name")
            and p["name"] not in seen_players
            and not seen_players.add(p["name"])
        ]
    ...
    def process_medals(data):
        for medal in data.get("podium", []):
            ...
            if rank == 1:
                medal_table[name]["gold"] += 1
            elif rank == 2:
                medal_table[name]["silver"] += 1
            elif rank == 3:
                medal_table[name]["bronze"] += 1
    ...
    def update_player_overall_stats(data):
        ...
        for player_data in data.get("players", []):
            ...
            games_in_this_tournament = (
                len(sheet_scores) if isinstance(sheet_scores, str) else 0
            )
            current_player_stats = player_overall_stats.setdefault(
                name,
                {
                    "total_score": 0,
                    "total_games": 0,
                    "tournaments_played_min_5_games": 0,
                },
            )
    ...
    def process_berserk(data):
        player_lookup = {
            p.get("name"): p for p in data.get("players", []) if p.get("name")
        }
        for p_podium in data.get("podium", []):
            ...
            if berserks_played_this_tourney == 0:
                continue
            ...
            if full_player_data:
                ...
                entry = berserk_table.setdefault(
                    name,
                    {
                        "berserk_games": 0,
                        "berserk_wins": 0,
                        "total_games_in_podium_summary": 0,
                    },
                )
                entry["berserk_games"] += berserks_played_this_tourney
                entry["berserk_wins"] += berserk_wins_counted
                entry["total_games_in_podium_summary"] += (
                    total_games_for_podium_player_this_tourney
                )
